...
<#
.SYNOPSIS
    v7 瀹為獙娴佹按绾匡細涓夐樁娈甸槻杩囨嫙鍚堬紙鍏ㄦ牱鏈啋骞村害鈫扺F锛夆啋 鍒嗘瀽
.DESCRIPTION
    鍩轰簬 V5 娴佹按绾夸慨澶嶇増銆備笉闇€瑕佷笅杞芥柊鏁版嵁锛堜娇鐢ㄥ凡鏈?甯佺锛夈€?    淇 V5 bug锛氬垎骞村害鍥炴祴姝ｇ‘浼犻€?strategy-path銆?#>
...
$StrategyPathDocker = "/freqtrade/user_data/strategies/experiments_v7"
$DefaultConfigDocker = "/freqtrade/user_data/config_backtest.json"

Set-Location $workDir

$manifestPath = "user_data/experiment_manifest_v7.json"
$resultsDir = "user_data/experiment_results"
$resultsCsv = "$resultsDir/v7_results.csv"
$yearlyCsv = "$resultsDir/v7_yearly.csv"
$wfCsv = "$resultsDir/v7_walkforward.csv"
$progressFile = "$resultsDir/v7_progress.txt"
$summaryFile = "$resultsDir/v7_pipeline_summary.txt"
...
# ========== Stage 1 鈥?Full-sample screen ==========
Write-PhaseBanner "STAGE 1 鈥?Full-sample screen ($fullSampleRange)"
...
if (Test-Path $resultsCsv) {
    ...
    Write-Host "Resume: $($completedStrategies.Count) strategies already in v7_results.csv" -ForegroundColor Yellow
}
else {
...
for ($i = 0; $i -lt $totalRemaining; $i++) {
    ...
    try {
        ...
        if ([double]$m.sharpe -ge 0.6) {
            ($rawOutput -join "`n") | Out-File "$resultsDir/v7_${strategy}_raw.txt" -Encoding utf8
        }
    }
    catch {
...
Write-PhaseBanner "STAGE 1.5 鈥?Gate filter (Sharpe>$s1MinSharpe PF>$s1MinPF trades>$s1MinTrades DD<$s1MaxDD%)"
...
if ($phase1Candidates.Count -eq 0) {
    Write-PhaseBanner "STAGE 2 鈥?SKIPPED (no S1 candidates)"
}
else {
    Write-PhaseBanner "STAGE 2 鈥?Yearly validation ($($phase1Candidates.Count) candidates x $($yearlyPeriods.Count) years)"
...
if ($phase2Survivors.Count -eq 0) {
    Write-PhaseBanner "STAGE 3 鈥?SKIPPED (no S2 survivors)"
}
else {
    Write-PhaseBanner "STAGE 3 鈥?Walk-Forward ($($phase2Survivors.Count) survivors x $($wfWindows.Count) windows)"
...
# ========== Analysis ==========
Write-PhaseBanner "ANALYSIS 鈥?Running analyze_v7.py"

$analyzeScript = "user_data/scripts/analyze_v7.py"
if (Test-Path $analyzeScript) {
    try {
        & python $analyzeScript
        Write-Host "Analysis complete. See experiment_results/v7_analysis.md" -ForegroundColor Green
    }
    catch {
    ...
    Write-Host "analyze_v7.py not found; skipping." -ForegroundColor Yellow
}
