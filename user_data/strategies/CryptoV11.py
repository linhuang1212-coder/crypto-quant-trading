"""
CryptoV11 Strategy - CryptoV10 + 动态DC周期(G6) + MACD共振(G2)
=============================================
基于 V4 三阶段实验（281个全样本→110分年度→110 Walk-Forward）筛选出的两个最优改进：

改进1 - 动态DC周期 (G6_P010)：
  根据近期ATR百分位动态选择DC周期：低波动(<70%百分位)用短周期14，高波动用长周期40。
  逻辑：低波动环境需要更敏感的突破信号，高波动环境需要更稳定的通道避免假突破。
  V4验证：Sharpe 1.15→1.18, PF 1.46→1.48, 利润+88%, 邻域std=0.00, OOS比率1.28

改进2 - MACD共振过滤 (G2_P019)：
  入场时要求MACD(8,17,9)柱状图为正且上升，确认短期动量方向与突破一致。
  V4验证：OOS比率1.33（全方向最高），样本外PF 1.53

其余保持 CryptoV10 不变：
  - 阶梯止损 S5: 20bars/-0.06 + 40bars/-0.04
  - trailing stop 0.03/0.30
  - 5x杠杆, 40%复利
"""
...
class CryptoV11(IStrategy):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # G6: 动态DC — 根据ATR百分位选择短/长DC周期
        atr_pct = dataframe['atr'].rolling(self.ATR_LOOKBACK).apply(
            lambda x: float(DataFrame(x).rank(pct=True).iloc[-1].item())
            if len(x) == self.ATR_LOOKBACK else np.nan,
            raw=False,
        )
    ...
    def _record_factor_snapshot(self, pair: str, rate: float, current_time):
        try:
            ...
            snapshot = {
                ...
                'macd_hist': round(float(last['macdhist']), 6),
            }
