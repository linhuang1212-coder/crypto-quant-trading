"""V6 E4b ADX decay exit drop=5 profit=0.03"""
...
class V6_E4_P016(IStrategy):
    ...
    def custom_exit(self, pair, trade, current_time, current_rate, current_profit, **kwargs):
        ...
        if bars_held >= 20 and current_profit < -0.06:
            return "stair_mid"
        if bars_held >= 8 and current_profit > 0.03:
            dataframe, _ = self.dp.get_analyzed_dataframe(pair, self.timeframe)
            if len(dataframe) >= 6:
                adx_vals = dataframe['adx'].iloc[-6:].values
                if all(adx_vals[i] > adx_vals[i+1] for i in range(5)):
