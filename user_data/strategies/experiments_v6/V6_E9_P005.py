"""V6 E9 ADX-DC dc_strong=12 dc_weak=35 adx_th=25"""
...
class V6_E9_P005(IStrategy):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        use_strong = dataframe['adx'] > 25
        dc_s = dataframe['high'].rolling(12).max().shift(1)
        dc_w = dataframe['high'].rolling(35).max().shift(1)
        dataframe['dc_upper'] = np.where(use_strong, dc_s, dc_w)
        dc_low_s = dataframe['low'].rolling(12).min().shift(1)
        dc_low_w = dataframe['low'].rolling(35).min().shift(1)
