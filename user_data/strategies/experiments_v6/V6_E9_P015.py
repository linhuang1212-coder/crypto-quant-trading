"""V6 E9 ADX-DC dc_strong=10 dc_weak=40 adx_th=35"""
...
class V6_E9_P015(IStrategy):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        use_strong = dataframe['adx'] > 35
        dc_s = dataframe['high'].rolling(10).max().shift(1)
        dc_w = dataframe['high'].rolling(40).max().shift(1)
        dataframe['dc_upper'] = np.where(use_strong, dc_s, dc_w)
        dc_low_s = dataframe['low'].rolling(10).min().shift(1)
        dc_low_w = dataframe['low'].rolling(40).min().shift(1)
